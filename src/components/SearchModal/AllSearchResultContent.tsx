...
const AllSearchResultContent = ({ inputValue }: AllSearchResultContentProps) => {
    ...
    const fetchSearchResults = (query: string) => {
        ...
        return apiReferenceData.filter((item) => item.title.toLowerCase().includes(lowercaseQuery));
    };
                                ...
                                <div key={item.id} className="results-item-container">
                                    <li>
                                        ...
                                        <div className="result-search-name">
                                            <span className="result-item-title">{highlightMatch(item.title, inputValue)}</span>
                                            <span className="result-item-group">{highlightMatch(item.group, inputValue)}</span>
                                        </div>
                                    </li>
                                </div>
                            ))}
