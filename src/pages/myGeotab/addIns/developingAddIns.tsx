...
// 
const GeotabAddInTypes: ReactNode = <div className="paragraph">
    <h3>Pages</h3>
    <p>A custom page Add-In can be thought of as a complete web application inside your Geotab account. A custom page Add-In has access to the MyGeotab API and the current page state. With custom pages you can develop business-aware Add-Ins by combining MyGeotab data with your own APIs.</p>
    <h3>Buttons</h3>
    <p>Custom button Add-Ins can be included to perform different functions.  Additional navigational buttons can be dynamically inserted inside certain areas of the MyGeotab user interface. This allows custom button Add-Ins to provide a simple way for users to reach your custom page Add-In (see Image 1).  Buttons can also be placed on pages to execute functions for automation of routine tasks, such as report generation (see Image 3).</p>
</div>;

// 
const UseCases: ReactNode = <div className="paragraph">
    <p>Consider a manager who regularly performs similar tasks each day. Their workflow may consist of comparing fleet metrics with local data from your software. By creating a custom page Add-In, you can combine both of these tasks into one easy-to-use page where all the information is readily available.</p>

    <p>Another example is to quickly navigate between different areas of Geotab. A simple-to-use one-click button Add-In can be made for most pages. The button will guide the user between areas, and can optionally use the page state to automatically fill in URL parameters to run reports with a single click.</p>
...
//
const AddInConfiguration: ReactNode = <div className="paragraph">
    <p>Each Add-In created will have one configuration file. The configuration file is a JSON file (<a href="http://www.json.org">http://www.json.org</a>) of keys and values which describes the Add-In, who is responsible for it, what source code it contains, and a digital security signature.</p>
    <h3>Table 1 — Add-In Configuration File Keys/ Values</h3>
...
//
const ExampleAddInConfiguration: ReactNode = <div className="paragraph">
    <p>The Add-In configuration file below demonstrates how to define a simple Add-In which references an HTML page specified by its URL. Any CSS or JavaScript which is required by the Add-In would be specified in the referenced HTML.</p>

    <h3>Listing 1 — A Simple Add-In Configuration JSON File</h3>
    ...
    <p>Test the example above by navigating to Administration → System → System Settings → Add-Ins. Select New Add-In and paste the example in the configuration tab. Be sure to select save then refresh the page.</p>

    <p>Each Add-In can create a navigation entry in the left hand side menu. This allows quick and easy access to all of the Add-Ins. The placement of the navigation entry for the Add-In is specified in the configuration file, relative to another built-in navigation entry. In the section below, a navigation entry is placed directly after the Activity entry. A user with English as their interface language will see "English Menu Text" as the label. Note that we show only two properties for brevity.</p>
    ...
    <ul>
        <li>GettingStartedLink</li>
        <li>ActivityLink</li>
        <li>EngineMaintenanceLink</li>
        <li>ZoneAndMessagesLink</li>
        <li>RuleAndGroupsLink</li>
        <li>AdministrationLink</li>
    </ul>

    <p>To place the navigation entry as a sub-menu entry in one of the main entries, place a slash (/) character after the name. The custom entry will be the first item inside the sub-menu.</p>

    <p>For example, by changing "ZoneAndMessagesLink/" as the value for the "path" key:</p>

    <CodeSample
        language="json"
        code={`"path": "ZoneAndMessagesLink/",`}></CodeSample>
    ...
    <div className="table-container">
        <table>
            ...
            <tbody>
                ...
                <tr>
                    <td>path</td>
                    <td>Specifies where in the menu hierarchy this menu item should reside. It will follow the menuId specified or become a child item if a trailing slash is provided, such as "ActivityLink/".</td>
                    <td>String</td>
                </tr>
                <tr>
                    <td>menuName</td>
                    <td>An object containing key value pairs for the text that appears on the menu item. The key is the language and the value is the text, for example: {`{"EN", "New menu item"}`}.</td>
                    <td>Object</td>
                </tr>
                <tr>
                    <td>icon</td>
                    <td>(To be deprecated June 04, 2021) A URL to the image (svg, png, jpg, etc.) that is placed in front of the menu item. Note that the current image size is 32x32 but it is recommended that SVG icons are used to allow for scaling. This property is to be deprecated and replaced by svgIcon for versions 2102 onward. During the transition period, if both icon and svgIcon exist, svgIcon will have higher priority (<a href="https://www.geotab.com/blog/mygeotab-add-in-icons-specs/">details here</a>).</td>
                    <td>String</td>
                </tr>
                <tr>
                    <td>svgIcon</td>
                    <td>A URL to the svg image that is placed in front of the menu item. Since the image file type is a vector, you only need to submit one file in any color. The icon file will be updated to the appropriate colors (<a href="https://www.geotab.com/blog/mygeotab-add-in-icons-specs/">details here</a>). A validator is available <a href="#add-in-icon-validator">here</a>.</td>
                    <td>String</td>
                </tr>
            </tbody>
        </table>
    ...
    <h3>Table 3 — Parent Menu Item</h3>
    ...
    <div className="table-container">
        <table>
            ...
            <tbody>
                <tr>
                    <td>menuId</td>
                    <td>A unique identifier for this menu. This string value of your choice but should be unique. See built-in ones above "GettingStartedLink", "ActivityLink", etc.</td>
                    <td>String</td>
                </tr>
                <tr>
                    <td>path</td>
...
const CreatingSubmenu: ReactNode = <div className="paragraph">
    ...
    <p>The process consists in creating a <a href="#table-3--parent-menu-item">parent menu item</a> with the menuName for the submenu item, a menuId, icon, and a path for one of the build-in path navigation values  (<em>GettingStartedLink</em>, <em>ActivityLink</em>, <em>EngineMaintenanceLink</em>, <em>ZoneAndMessagesLink</em>, <em>RuleAndGroupsLink</em>, <em>AdministrationLink</em>).</p>

    <p>To place a <a href="#table-2--menu-item">menu item</a> under a parent menu item you will use the unique ID of the submenu as a path for the item. This is illustrated in the sample configuration below:</p>
    ...
    <h3>Referencing Source Items</h3>

    <p>Each Add-In can define zero or more <em>items</em> as part of its configuration file. An item is a collection of keys and values which represent a page or a button.</p>

    <h3>Table 4 — Button Item</h3>
    ...
    <div className="table-container">
        <table>
            ...
            <tbody>
                <tr>
                    <td>page</td>
                    <td>Which built-in page to place the button on. Possible values: 'map', 'tripsHistory', 'devices', 'device', 'zones', 'users', 'user', 'rules', 'rule', 'exceptions', 'customReports', 'customReport', 'engineFaults', 'speedProfile', 'hosLogs', 'hosLog', 'groupsTree', 'routes', 'fuelUsage', and 'engineMeasurements'.</td>
                    <td>String</td>
                </tr>
                <tr>
                    <td>click</td>
                ...
                <tr>
                    <td>buttonName</td>
                    <td>An object containing key value pairs for the text that appears on the button. The key is the language, and the value is the text, for example {`{"EN", "New menu item"}`}.</td>
                    <td>Object</td>
                </tr>
                <tr>
                    <td>icon</td>
                    <td>(To be deprecated June 04, 2021) for placing it in the button label. This property is to be deprecated and replaced by svgIcon for versions 2102 onward. During the transition period, if both icon and svgIcon exist, svgIcon will have higher priority (<a href="https://www.geotab.com/blog/mygeotab-add-in-icons-specs/">details here</a>).</td>
                    <td>String</td>
                </tr>
                <tr>
                    <td>svgIcon</td>
                    <td>A URL Reference to the svg image for placing it in the button label. Since the image file type is a vector, you only need to submit one file in any color. The icon file will be updated to the appropriate colors (<a href="https://www.geotab.com/blog/mygeotab-add-in-icons-specs/">details here</a>). A validator is available <a href="#add-in-icon-validator">here</a>.</td>
                    <td>String</td>
                </tr>
            </tbody>
        </table>
    ...
    <CodeSample
        language="javascript"
        code={`geotab.addin.myaddin = () => { ... }`}></CodeSample>

    <p>The name you provide should be unique for each Add-In and should take care to avoid including invalid characters in the name. Additionally, when referencing Add-Ins hosted externally, the absolute path to the Add-In should not include the following characters anywhere in their URL:</p>
    ...
    <p>When developing a custom page or button Add-In, you have the option to embed the source code for your project in the JSON configuration file. When using this method, there is no requirement to host your own HTML, CSS, or JavaScript files as they will be converted into strings and written inside the configuration file itself.</p>

    <h3>Listing 3 — Add-In Configuration File Using Embedded Source Code</h3>
    ...
    <h3>Listing 4 — Absolute Path to Image in HTML</h3>

    <p>Referencing an external image using an absolute URL in the HTML or CSS.</p>

    <CodeSample
        language="html"
        code={`<img src="https://mycompany.com/images/customImage.png" alt="CustomImage" />`}></CodeSample>

    <h3>Listing 5 — Absolute Path to Image in CSS</h3>
    ...
    <h3>Listing 6 — Add-In Configuration File with Base64 Encoded Image</h3>

    <p>The other method is to embed the images along with the rest of the source code in the markup. First, the images will need to be encoded using <a href="http://en.wikipedia.org/wiki/Base64">Base64 encoding</a>, then the references to the image files replaced with the encoded version directly in the HTML or CSS.</p>
...
<html>
    ...
    <p>Keep in mind that the end-user using the Add-In will be viewing your Add-In from a Geotab server. The files referenced must be publicly available on the Internet. Firewalls cannot be configured with a specific IP address for the Geotab server as the address may change in the future.</p>
    ...
    <h3>Listing 8 — Avoiding CSS Naming Conflicts</h3>

    <p>When referencing the CSS files, keep in mind that naming conflicts are possible. Geotab's outer framework defines a number of CSS styles for common HTML tags which the custom Add-In may inherit. When designing HTML tags, it is recommended to prefix the HTML tags with a common name when they are stylized with the CSS.</p>

    <CodeSample
        language="html"
        code={`<div id="myaddin-menu">
...
//
const PageLifecycle: ReactNode = <div className="paragraph">
    <p>When designing a custom page Add-In, it is important to understand the JavaScript events that take place on Geotab's servers. The user-defined JavaScript code must supply an entry point object which will be created with a predefined parameter signature. From these parameters, a signed-in Geotab API object will be received, along with the page state and a callback synchronization function.</p>
...
//
const CustomButtonAddIns: ReactNode = <div className="paragraph">
    <p>Custom button Add-Ins allow you to extend the capabilities of a built-in Geotab page by appending a new button to the top menu of the page. When a user clicks the custom button Add-In, the JavaScript method that has been defined will be called. This JavaScript method has access to the event which was generated by the click, the Geotab API under the credentials of the current user, and the page state variables.</p>

    <h3>Button Add-In Configuration File</h3>

    <p>The following example adds a custom button Add-In to the live map page. When clicked, the button will redirect the user to the vehicles page. Use this example as a starting point for creating your own custom button Add-Ins.</p>
    ...
    <h3>Listing 10 — Custom Button Add-In Configuration File</h3>

    <p>Almost any page is available to have a custom button Add-In added to it. Use a web browser's address bar to find the correct value for the Page property. Geotab pages will have a trailing hash (“#”) symbol followed by the page name. Determine the page name and then set the “page” value of your custom button Add-In configuration file to that value.</p>
...
const JavaScriptButtonAction: ReactNode = <div className="paragraph">
    ...
    <h3>Table 6 - Geotab Page State Methods</h3>
...
const CompleteIntegrationExample: ReactNode = <div className="paragraph">
    ...
    <h4>Listing 13 — HTML and JavaScript code for integration example</h4>
...
<html>
    ...
    <h4>Listing 14 — Configuration file for integration example</h4>
...
const TroubleshootingAndDebugging: ReactNode = <div className="paragraph">
    ...
    <p><strong>Note:</strong> Add-Ins that were locally added to a database in the past (the recommended approach is to host on a server externally) might throw the error message <strong>"Add-In threw an error. Please contact your administrator."</strong> To further confirm, the following error will appear in the browser console (found by hitting CTRL+SHIFT+I): <strong>Error: Add-In files: &lt;add-in file name&gt; couldn't be loaded. Probably they were moved to another location or removed.</strong> This could be linked to a server maintenance/migration event. The workaround would be to re-upload the files for the Add-Ins back to the database, while the resolution would be to externally host the source code as per the requirements listed <a href="https://geotab.github.io/sdk/software/guides/developing-addins/#requirements">here</a>.</p>
</div>;

//
const AddInIconValidator: ReactNode = <div className="paragraph">
    <p>The Add-In Icon Validator tests uploaded SVG files against <a href="https://www.geotab.com/blog/mygeotab-add-in-icons-specs/">Geotab's requirements</a> and displays them in the reformatted colors and style that will be displayed in MyGeotab. <a href="../developing-addins/addin-icon-validator.html">Launch the validator</a>.</p>
</div>;
...
export default function DevelopingAddIns() {
    return (
        <div className="grayBackground pageContent">
            ...
            <Accordion summary="Add-In Configuration Files" p={AddInConfiguration}></Accordion>
            <Accordion summary="Example Add-In Configuration File" p={ExampleAddInConfiguration}></Accordion>
            <Accordion summary="Listing 2 — Creating Submenu Items" p={CreatingSubmenu}></Accordion>
            <Accordion summary="Using Third-Party Libraries" p={ThirdPartyLibraries}></Accordion>
            <Accordion summary="Page Lifecycle" p={PageLifecycle}></Accordion>
            <Accordion summary="Custom Button Add-Ins" p={CustomButtonAddIns}></Accordion>
            <Accordion summary="JavaScript Button Action" p={JavaScriptButtonAction}></Accordion>
            <Accordion summary="Complete Integration Example" p={CompleteIntegrationExample}></Accordion>
            <Accordion summary="Troubleshooting and Debugging" p={TroubleshootingAndDebugging}></Accordion>
            <Accordion summary="Add-In Icon Validator" p={AddInIconValidator}></Accordion>
        </div>
    );
};
