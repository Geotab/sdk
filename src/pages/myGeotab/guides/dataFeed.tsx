...
const searching: ReactNode = (
  <div className="paragraph">
    ...
    <p>
    Do not pass a search to any feed unless it specifically is mentioned in the <a href="https://geotab.github.io/sdk/software/api/reference/#GetFeed1">
        GetFeed method
      </a> {/* TODO: Need to replace this with routing to api/reference/GetFeed reference when created*/} documentation.
    </p>
  </div>
);
...
const activeVsCalculated: ReactNode = (
 ...
 <p>
      ...
      <p>
        <strong>Active data</strong> are records that are received from a source, usually a Go device or user input. They are not created based off of other data. These records are static; once received, they are typically not updated and will not be removed by the system.
        For example, as new GPS (LogRecord) data arrives from a device, it is stored by the system.
      </p>

      <p>
        <strong>Calculated data</strong> are records generated automatically by the system, usually in response to active data that has recently been received. These records are dynamic; an existing calculated data record can be edited and removed by the system automatically, based on new active data that was received. This processing happens in real-time as new active data is received by the system.
...
const trips: ReactNode = (
  <div className="paragraph">
    ...
    <ul>
      ...
      <li>
      the new trip’s stop duration is 
      </li>
    </ul>
  </div>
);
...
export default function DataFeed() {
  return (
    <div className="pageContent">
      ...
      <Accordion summary="Next Steps" p={cachingToImprovePerformance} />
