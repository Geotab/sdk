...
For a constant poll, we recommend polling for data on a 30 second interval. However, we understand for some use cases, this to too infrequent to keep up with the rate of data generated by your vehicles and have created limits accordingly. Rate limits of **1 request-per-second** are applied to `GetFeed` requests of each supported Entity type indipendantly.
