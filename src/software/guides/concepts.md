...
API rate limits are restrictions imposed on the number of requests a client can make to an API within a specified time period. These limits are in place to prevent abuse, ensure fair usage, and maintain the overall performance and reliability of the API.

#### How Rate Limits Work
**Limits**: The number of requests users are allowed to make within a specific time window.

**Period**: The time window where the limits are going to be applied (e.g., 100 requests per minute).

**Status**: Indicates whether the rule is active or not. Possible status are:
- **Active**: The rule is active and if the limit is exceeded, further requests are going to be rejected;
- **Coming soon**: The rule is in place, all the headers are going to be present but still, it is not going to block calls that exceed the limit. This a time frame that users have to adapt their integration to work with the rate limit rule.
