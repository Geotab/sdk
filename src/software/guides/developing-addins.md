...
The Add-Ins created can either have their source code embedded and stored on Geotab's servers or can be stored externally using your own hosting provider or your own servers. Note that any referenced files must be publicly accessible and all hosted resources must be on a server that supports TLS 1.2 or higher.
